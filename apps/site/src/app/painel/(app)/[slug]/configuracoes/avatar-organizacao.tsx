...
import { UploadAvatarOrganizacaoAction } from './actions'

interface AvatarOrganizacaoProps {
  organizacao: Organizacao
  size: number
}

export default function AvatarOrganizacao({
  organizacao,
  size,
}: AvatarOrganizacaoProps) {
  ...
  async function uploadAvatar(file: File) {
    ...
    if (file.type.lastIndexOf('image/') < 0) {
      ...
      const result = await UploadAvatarOrganizacaoAction(
        organizacao.id,
        file,
